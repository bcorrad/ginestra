...
def objective(trial, train_loader, val_loader, in_channels, out_channels, config_idx):
    ...
    for run in range(N_RUNS):
        ...
        for epoch in range(GRID_N_EPOCHS):
            ...
            log_train = f"[GAT TRAINING RUN {run+1}/{N_RUNS} E{epoch+1}] Loss: {train_loss:.4f}, P: {precision:.4f}, R: {recall:.4f}, F1: {f1:.4f}"
            log_val = f"[GAT VALIDATION RUN {run+1}/{N_RUNS} E{epoch+1}] Loss: {val_loss:.4f}, P: {val_precision:.4f}, R: {val_recall:.4f}, F1: {val_f1:.4f}"
            print(log_train)
            print(log_val)
            with open(report_file, "a") as f:
                f.write(log_train + "\n")
