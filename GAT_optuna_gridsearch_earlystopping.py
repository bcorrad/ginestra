...
# from config_gridsearch import GRID_N_EPOCHS, LABELS_CODES, EXPERIMENT_FOLDER, N_RUNS, USE_FINGERPRINT
...
EXPERIMENT_FOLDER = initialize_experiment("gat")
...
def objective(trial, train_loader, val_loader, in_channels, out_channels, config_idx):
    ...
    for run in range(N_RUNS):
        ...
        for epoch in range(GRID_N_EPOCHS):
            ...
            log_train = f"[CONFIG {config_idx}][GAT TRAINING RUN {run+1}/{N_RUNS} E{epoch+1}] Loss: {train_loss:.4f}, P: {precision:.4f}, R: {recall:.4f}, F1: {f1:.4f}"
            log_val = f"[CONFIG {config_idx}][GAT VALIDATION RUN {run+1}/{N_RUNS} E{epoch+1}] Loss: {val_loss:.4f}, P: {val_precision:.4f}, R: {val_recall:.4f}, F1: {val_f1:.4f}"
            print(log_train)
            print(log_val)
            with open(report_file, "a") as f:
                f.write(log_train + "\n")
...
def optuna_grid_search(train_loader, val_loader, test_loader, in_channels, out_channels):
    ...
    def wrapped_objective(trial):
        ...
        return objective(trial, train_loader, val_loader, in_channels, out_channels, config_idx)
