...
def calculate_fingerprint(smiles, radi):
    binary = np.zeros((2048*(radi)), int) # Questo è un vettore di dimensione 2048 * radii, dove ogni raggio (1, 2, ...) ha un set di bit.
    formula = np.zeros((2048),int)  #Questo è un vettore di dimensione 2048, che terrà traccia di quante volte ogni bit del fingerprint è attivo al raggio 0.
